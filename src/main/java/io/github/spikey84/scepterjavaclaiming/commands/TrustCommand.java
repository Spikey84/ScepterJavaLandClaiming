...
public class TrustCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            ChatUtil.message(player, "Enter a player to trust.");
            return true;
        }
        ...
        for (Claim claim : claimManager.getClaims()) {
            ...
            if (!claim.getOwner().equals(player.getUniqueId())) {
                ChatUtil.message(player, "You must be the owner of this claim to add a trusted member.");
                return true;
            }
            ...
            if (claimManager.getTrustedMembers().get(claim).contains(target)) {
                ChatUtil.message(player, "This player is already trusted in this claim.");
                return true;
            }
            claimManager.getTrustedMembers().get(claim).add(target);
            ChatUtil.message(player, "This player is now trusted in your claim.");
            return true;
        }
